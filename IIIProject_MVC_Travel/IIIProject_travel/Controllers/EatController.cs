...
namespace IIIProject_travel.Controllers
{
    public class EatController : Controller
    {
        ...
        public ActionResult article_AJAX(string condition)
        {
            ...
            var FinalList = AJAXcondition(condition).Where(a => a.f活動類型 == "飯局").Select(a => a);
            CEat List = new CEat();
        ...
        [HttpPost]
        public ActionResult EatIndex(int? id) //HomeSearch才用
        {
            string HomeSearch = "";
            HomeSearch += Request.Form["txtEatKeyword"];
            HomeSearch += "," + Request.Form["txtEatCategory"];
            ...
            if (!string.IsNullOrEmpty(NowMember.f會員已占用時間))
            {
                ...
                foreach (var item in usedTime)
                {
                    if (!string.IsNullOrEmpty(item))
                    {
                        ...
                        {
                            return RedirectToAction("EatIndex", "Eat", new { msg = "錯誤! 修改的活動時間與既有活動時間相衝" });
                        }
                    }
        ...
        {
            var autoComplete = db.tActivity.Where(t => t.f活動類型 == "飯局").Select(t => t.f活動標題).ToArray();
